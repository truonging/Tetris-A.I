...
class Linear_QNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.output_layer(x)  # Linear output layer
        return x
...
class QTrainer:
    ...
    def train_step(self):
        ...
        state_batch = torch.as_tensor(states, dtype=torch.float32)
        next_state_batch = torch.as_tensor(next_states, dtype=torch.float32)
        reward_batch = torch.as_tensor(rewards, dtype=torch.float32).view(-1, 1)
        done_batch = torch.as_tensor(dones, dtype=torch.bool)
